...
public class vision : MonoBehaviour
{
    ...
    void Update()
    {
        initialX = destination.position.x - this.gameObject.transform.position.x;
        initialY = destination.position.y - this.gameObject.transform.position.y;
        x = (float)Math.Atan((initialY)/(initialX));
        if(destination.position.x < this.gameObject.transform.position.x) {
            this.gameObject.transform.eulerAngles = new Vector3(0,0,-90+(float)(x * 180/3.1415926535898));
        }
        else {
            this.gameObject.transform.eulerAngles = new Vector3(0,0,90+(float)(x * 180/3.1415926535898));
        }
